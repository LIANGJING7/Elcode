...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@/plugin/deps/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@/tui/deps/util/error"
import { isRecord } from "@/tui/deps/util/record"
...
import { hasTheme, upsertTheme } from "@/tui/deps/context/theme"
...
import type { HostPluginApi, HostSlots } from "@/tui/deps/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@/core/v1/config/plugin"
import { createCommandShim } from "@/tui/deps/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@/tui/deps/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".lcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".lcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".lcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.LCODE_PURE ? [] : pluginOrigins
    if (Flag.LCODE_PURE && pluginOrigins.length) {
    }
