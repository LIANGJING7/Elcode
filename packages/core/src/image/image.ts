...
import photonWasmUrl from "@silvia-odwyer/photon-node/photon_rs_bg.wasm?url"
  ...
  Effect.gen(function* () {
    ...
    const loadPhoton = yield* Effect.cached(
      Effect.sync(() => {
        ...
        ;(globalThis as typeof globalThis & { __LCODE_PHOTON_WASM_PATH?: string }).__LCODE_PHOTON_WASM_PATH =
          path.isAbsolute(photonWasmUrl) ? photonWasmUrl : fileURLToPath(new URL(photonWasmUrl, import.meta.url))
      }).pipe(
