...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in lcode.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .lcode/ subdirectory too)
  const candidates = [path.join(baseDir, "lcode.json"), path.join(baseDir, "lcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".lcode", "lcode.json"), path.join(baseDir, ".lcode", "lcode.jsonc"))
  }
  ...
  // Default to lcode.json if none exist
  return candidates[0]
}
