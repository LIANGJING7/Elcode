...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "lcode.json", "lcode.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".lcode", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".lcode")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".lcode").toReversed()
    ...
    // global config, project files, then `.lcode` files.
