...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from lcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const lcodeFiles = await lcodeFiles(input)
  for (const file of lcodeFiles) {
...
async function lcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "lcode"),
    ...(await Filesystem.findUp(["lcode.json", "lcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "lcode"))
  }
  if (Flag.LCODE_CONFIG) files.push(Flag.LCODE_CONFIG)
