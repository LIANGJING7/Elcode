...
import type { ConsoleState } from "@/core/v1/config/console-state"
...
import { ConfigPermissionV1 } from "@/core/v1/config/permission"
import { ConfigPluginV1 } from "@/core/v1/config/plugin"
...
function globalConfigFile() {
  const candidates = ["lcode.jsonc", "lcode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.LCODE_CONFIG && !Flag.LCODE_CONFIG_DIR && !Flag.LCODE_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "lcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "lcode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "LCODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.LCODE_CONFIG) {
          yield* merge(Flag.LCODE_CONFIG, yield* loadFile(Flag.LCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.LCODE_CONFIG })
        }

        if (!Flag.LCODE_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.LCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from LCODE_CONFIG_DIR", { path: Flag.LCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".lcode") || dir === Flag.LCODE_CONFIG_DIR) {
            for (const file of ["lcode.json", "lcode.jsonc"]) {
        ...
        if (process.env.LCODE_CONFIG_CONTENT) {
          const source = "LCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.LCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from LCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["LCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("LCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.LCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.LCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("LCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.LCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.LCODE_DISABLE_PRUNE) {
