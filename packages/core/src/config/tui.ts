...
import { TuiKeybind } from "@/tui/deps/config/keybind"
...
import { TuiConfig } from "@/tui/deps/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and LCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.LCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit LCODE_TUI_CONFIG override, if set.
  if (Flag.LCODE_TUI_CONFIG) {
    const configFile = Flag.LCODE_TUI_CONFIG
  ...
  // 4. `.lcode` directories (and LCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".lcode") || dir === Flag.LCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".lcode") && dir !== Flag.LCODE_CONFIG_DIR) continue
