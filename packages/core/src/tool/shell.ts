...
const parser = lazy(async () => {
  ...
  const treeWasmUrl = await import("web-tree-sitter/tree-sitter.wasm?url")
  const treePath = resolveWasm(treeWasmUrl.default)
  await Parser.init({
    locateFile() {
      return treePath
    },
  })
  const bashWasmUrl = await import("tree-sitter-bash/tree-sitter-bash.wasm?url")
  const psWasmUrl = await import("tree-sitter-powershell/tree-sitter-powershell.wasm?url")
  const bashPath = resolveWasm(bashWasmUrl.default)
  const psPath = resolveWasm(psWasmUrl.default)
